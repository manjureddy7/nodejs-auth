...
module.exports = {
  generateAccessToken: (userid) => {
    return new Promise((resolve, reject) => {
      ...
      const secrect =
        process.env.ACCESS_TOKEN_SECRET_KEY || "some_super_secret_key";
      const options = {
        expiresIn: "45s",
        issuer: "google.com",
        audience: userid,
  ...
  generateRefreshToken: (userid) => {
    return new Promise((resolve, reject) => {
      ...
      const secrect =
        process.env.REFRESH_TOKEN_SECRET_KEY || "some_super_secret_refresh_key";
      const options = {
        expiresIn: "1y",
        issuer: "google.com",
        audience: userid,
  ...
  verifyAccessToken: (req, res, next) => {
    ...
    JWT.verify(token, process.env.ACCESS_TOKEN_SECRET_KEY, (err, payload) => {
      if (err) {
        ...
        const message =
          err.name === "JsonWebTokenError" ? "Unauthoried" : err.message;
